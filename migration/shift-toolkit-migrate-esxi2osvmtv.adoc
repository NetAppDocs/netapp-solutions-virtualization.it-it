...
* I VMDK possono essere configurati in diversi modi: tutti i VMDK possono risiedere su un singolo volume (eliminando la necessità di Storage vMotion), ciascun VMDK può essere posizionato su un volume individuale oppure più VMDK possono essere organizzati all'interno di un qtree in un volume.
+

NOTE: Il toolkit Shift rileva automaticamente il layout e seleziona il metodo di clonazione e i driver di storage NAS appropriati.
