...
Le VM devono essere spente correttamente durante la finestra di manutenzione pianificata.  Prima di avviare la migrazione, verificare che tutti i prerequisiti siano soddisfatti:

* Assicurarsi che la VM di Shift Toolkit faccia parte del dominio
* Assicurarsi che la condivisione CIFS sia configurata con le autorizzazioni appropriate
* Verificare che il qtree utilizzato per la migrazione o la conversione abbia lo stile di sicurezza corretto
* Testare la creazione della VM utilizzando Hyper-V Manager da qualsiasi host Hyper-V all'interno del cluster e posizionare il VHDx sulla condivisione CIFS
* Testare la creazione di VM dalla VM di Shift Toolkit aggiungendo gli strumenti di gestione Hyper-V tramite Programmi e funzionalità o PowerShell: `add-windowsfeature rsat-hyper-v-tools`
