...
[role="lead"]
Utilizza Shift Toolkit per convertire i dischi delle macchine virtuali VMware ESX (VMDK) nel formato disco Microsoft Hyper-V (VHDX) o nei formati disco RAW/QCOW2. Questo processo include la configurazione dei gruppi di risorse, la creazione di modelli di conversione e la pianificazione delle conversioni.



== Panoramica

Shift Toolkit supporta la conversione a livello di disco dei dischi virtuali tra hypervisor per i seguenti formati, con supporto sia per i volumi FlexVol che per i volumi FlexGroup su ONTAP:
...
I file qcow2 convertiti sono compatibili con qualsiasi hypervisor KVM. Ad esempio, un file qcow2 può essere utilizzato con KVM basato su RHEL tramite virt-manager per creare una VM, così come con KVM basato su Ubuntu, Rocky Linux e altri. Lo stesso può essere usato con Oracle Linux virtualization manager, Proxmox, OpenNebula, HP VME e con la virtualizzazione OpenShift dopo l'importazione tramite NetApp Trident. L'obiettivo è fornire il disco (convertito in secondi o minuti) che può poi essere integrato negli script di automazione esistenti usati dalle organizzazioni per effettuare il provisioning della VM e assegnare la rete. Questo approccio aiuta a ridurre i tempi complessivi di migrazione, con la conversione del disco gestita dalle API di Shift toolkit e il resto dello script che avvia le VM.

Il NetApp Shift Toolkit supporta anche la conversione dei dischi VMware VMDK archiviati su volumi FlexGroup in formato disco RAW, permettendoti una migrazione senza interruzioni verso piattaforme basate su KVM come Proxmox o OLVM.


NOTE: Quando si utilizza lo storage FlexGroup, sono supportate solo le conversioni in formato RAW. Inoltre, Shift toolkit inietta i driver VirtIO durante la preparazione quando si eseguono conversioni solo KVM.
