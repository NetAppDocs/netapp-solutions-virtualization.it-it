...
I dischi persistenti delle OpenShift Virtualization VM possono essere supportati dallo storage ONTAP integrato nel OpenShift Cluster utilizzando link:https://docs.netapp.com/us-en/trident/["Trident CSI"]. In questa sezione utilizziamo link:https://docs.netapp.com/us-en/trident/trident-protect/learn-about-trident-protect.html["Trident Protect"] per creare snapshot e backup delle VM, inclusi i relativi volumi di dati, su ONTAP Object Storage.

Quando necessario, eseguiamo il ripristino da uno snapshot o da un backup.

Trident Protect consente snapshot, backup, ripristino e disaster recovery di applicazioni e VM su un OpenShift cluster. Per le VM di OpenShift Virtualization, i dati che possono essere protetti con Trident Protect includono oggetti di risorse Kubernetes associati alle VM, volumi persistenti e immagini interne.
...
[%collapsible%open]
====
Prima di creare le snapshot e i backup per un'applicazione o una VM, un Object Storage deve essere configurato in Trident Protect per archiviare le snapshot e i backup. Questo viene fatto utilizzando il bucket CR. Solo gli amministratori possono creare un bucket CR e configurarlo. Il bucket CR è noto come AppVault in Trident Protect. Gli oggetti AppVault sono la rappresentazione dichiarativa del flusso di lavoro Kubernetes di un bucket di storage. Un CR AppVault contiene le configurazioni necessarie affinché un bucket possa essere utilizzato nelle operazioni di protezione, come backup, snapshot, operazioni di ripristino e replica SnapMirror.
