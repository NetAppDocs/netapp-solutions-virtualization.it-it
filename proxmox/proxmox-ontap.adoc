...
* Estendi il cluster su domini di errore con SnapMirror ActiveSync. Controllare sempre https://mysupport.netapp.com/matrix/#welcome["Strumento di matrice di interoperabilità"] per le configurazioni supportate.
...
. Per abilitare l'opzione nConnect, vai alla shell su uno qualsiasi dei nodi del cluster e digita pvesm set pvenfs01 --options nconnect=4 dove pvenfs01 è l'ID di archiviazione creato nel passaggio precedente. Si noti che se si prevede di utilizzare la funzionalità di trunking, assicurarsi che venga utilizzato almeno NFS v4.1 e impostare l'opzione trunkdiscovery. pvesm set pvenfs01 --options vers=4.1,trunkdiscovery
